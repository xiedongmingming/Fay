                        ...
                        if type == 1:
                            if js['Type'] == 'ReqProxy':
                                ...
                                if self.remoteSocket:
                                    thread = threading.Thread(target=self.HKClient,
                                                              args=(self.remoteSocket, 0, 2))  # 远程客户端已经连接，监测本地应用连接
                            ...
                            if js['Type'] == 'AuthResp':
                                ...
                                for info in self.tunnels:
                                    ...
                                    self.send_pack(sock, self.req_tunnel(reqid, info['protocol'], info['hostname'],
                                                                         info['subdomain'], info['httpauth'],
                                                                         info['rport']))
                        ...
                        if type == 2:
                            if js['Type'] == 'StartProxy':
                                ...
                                if self.localSocket:  # 本地应用连接成功
                                    thread = threading.Thread(target=self.HKClient,
                                                              args=(self.localSocket, 0, 3, sock))  # 本地应用已经连接，启用数据转发
                                    thread.setDaemon(True)
                                    thread.start()
